...
def _read_json_lines(file_path: str) -> list:
    records = []
...
def getResults():
    # Load list of exam results from newline-delimited JSON file
    return _read_json_lines('result.json')

def getResultDetails(result_id):
    # Load violation details for a specific result id
    details = []
    rid = str(result_id)
    records = _read_json_lines('violation.json')
    for rec in records:
